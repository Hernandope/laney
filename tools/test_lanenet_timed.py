...
def test_lanenet_batch(image_dir, weights_path, batch_size, use_gpu, save_dir=None):
    ...
    with sess.as_default():
        ...
        for epoch in range(epoch_nums):
            ...
            t_cost = time.time() - t_start
            prediction_time_list.append(t_cost)
            ...
            for index, binary_seg_image in enumerate(binary_seg_images):
                ...
                cluster_time_list.append(time.time() - t_start)
    ...
    print('The following are the time results:\n\n')
    print('read_time: {:.5f}\n'.format(mean(read_time_list)))
    print('prediction_time: {:.5f}\n'.format(mean(prediction_time_list)))
    print('cluster_time: {:.5f}\n'.format(mean(cluster_time_list)))
    print('write_time: {:.5f}\n'.format(mean(write_time_list)))
    print('number of items:\n\n')

    print('read list length: {:.5f}\n'.format(len(read_time_list)))
    print('prediction list length: {:.5f}\n'.format(len(prediction_time_list)))
    print('cluster list length: {:.5f}\n'.format(len(cluster_time_list)))
    print('write list length: {:.5f}\n'.format(len(write_time_list)))
