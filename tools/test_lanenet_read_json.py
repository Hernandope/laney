...
def init_args():
    ...
    parser.add_argument('-j','--test_json_path', type=str, help='Path to json file of test image')
...
### == EXTRACT DEM LANNEY GOODNESS for regular images==
def laneExtractorRaw(colored_lane_image):
...
### == reads the given test_json*.json file and returns opened json_data==
def testJsonReader(file_path):
    # with open(file_path, 'r') as f:
    #     json_data = json.load(f)
    json_data = [json.loads(line) for line in open(file_path).readlines()]
    return json_data


### == EXTRACT DEM LANNEY GOODNESS for tuSimple test images according to preset h_samples (cheat)==
def laneExtractorTuTest(colored_lane_image,h_samples_list,index):
    lane_colours = coloredLaneIdentifierRaw(colored_lane_image)
    lanes_dict = {}
    lanes = []
    ...
    h_samples = h_samples_list[index]
    for x in h_samples:
...
def testLanenetTuTest(weights_path, use_gpu, batch_size, test_json_path, save_dir=None):
    """
    :param image_dir:
    :param weights_path:
    :param batch_size:
    ...
    test_json = testJsonReader(test_json_path)
    json_directory = os.path.dirname(test_json_path)
    for json_data in test_json:
        image_path_list.append(json_directory+"/"+json_data['raw_file'])
        h_samples_list.append(json_data['h_samples'])
    ...
    with sess.as_default():
        ...
        for epoch in range(epoch_nums):
            ...
            for index, binary_seg_image in enumerate(binary_seg_images):
                ...
                lanes, h_samples = laneExtractorTuTest(mask_image,h_samples_list,index)
    ...
    with sess.as_default():
        ...
        for epoch in range(epoch_nums):
            ...
            for index, binary_seg_image in enumerate(binary_seg_images):
                ...
                lanes, h_samples = laneExtractorRaw(mask_image)
...
def testLanenetTuTestDirect(weights_path, use_gpu, test_json_path, save_dir=None):
    """
    :param image_dir:
    :param weights_path:
    :param batch_size:
    ...
    with sess.as_default():
        ...
        # for epoch in range(epoch_nums):
            ...
            image_path = json_directory+"/"+epoch['raw_file']
            image = cv2.imread(image_path, cv2.IMREAD_COLOR)
            image = cv2.resize(image, (512, 256), interpolation=cv2.INTER_LINEAR)
            image = image - VGG_MEAN
...
if __name__ == '__main__':
    ...
    if args.is_batch.lower() == 'false':
        ...
        testLanenetTuTest(weights_path=args.weights_path, use_gpu=args.use_gpu, batch_size=args.batch_size, test_json_path=args.test_json_path, save_dir=args.save_dir)
