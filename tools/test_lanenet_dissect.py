    ...
    with sess.as_default():
        ...
        for epoch in range(epoch_nums):
            ...
            for index, binary_seg_image in enumerate(binary_seg_images):
                t_start = time.time()
                ####saves binary image before preprocessing
                binary_seg_image_before= binary_seg_image
                for x in range(0,len(binary_seg_image)):
                    for y in range(0,len(binary_seg_image[x])):
                        if binary_seg_image[x][y] == 1:
                            binary_seg_image_before[x][y] = 255
                cv2.imwrite('binary_seg_image_before.jpg',binary_seg_image_before)


                binary_seg_image = postprocessor.postprocess(binary_seg_image)

                ####saves binary image after preprocessing
                binary_seg_image_after= binary_seg_image
                for x in range(0,len(binary_seg_image)):
                    for y in range(0,len(binary_seg_image[x])):
                        if binary_seg_image[x][y] == 1:
                            binary_seg_image_after[x][y] = 255
                cv2.imwrite('binary_seg_image_after.jpg',binary_seg_image_after)
                mask_image = cluster.get_lane_mask(binary_seg_ret=binary_seg_image,
                                                   instance_seg_ret=instance_seg_images[index])
                cv2.imwrite('mask_image_before.jpg',mask_image)
